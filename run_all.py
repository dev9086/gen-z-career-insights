...
def run(save_dir='artifacts'):
    ...
    if 'PursueHigherEd' in info.columns:
        ...
        with open(os.path.join(save_dir, 'genz_model_highered.pkl'), 'wb') as f:
            pickle.dump(model_1, f)
        try:
            fpr, tpr, _ = roc_curve(y1_test, model_1.predict_proba(X1_test)[:, 1])
            plt.figure()
            plt.plot(fpr, tpr, label=f'ROC (AUC={roc_1:.3f})')
            plt.plot([0,1],[0,1],'k--')
            plt.xlabel('FPR')
            plt.ylabel('TPR')
            plt.title('GenZ HigherEd ROC')
            plt.legend()
            plt.tight_layout()
            plt.savefig(os.path.join(save_dir, 'roc_genz_highered.png'))
            plt.close()
        except Exception:
            pass
    ...
    if 'StayWithEmployer3Years' in info.columns:
        ...
        with open(os.path.join(save_dir, 'genz_model_loyalty.pkl'), 'wb') as f:
    ...
    if 'CareerSatisfaction' in info.columns:
        ...
        with open(os.path.join(save_dir, 'genz_model_satisfaction.pkl'), 'wb') as f:
